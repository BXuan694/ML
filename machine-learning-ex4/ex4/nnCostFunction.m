...
%
out=sigmoid([ones(m,1) sigmoid([ones(m,1) X]*Theta1')]*Theta2');
ymat=zeros(m,num_labels);

for i=1:m
    ymat(i,y(i))=1;
end
J=sum(sum(-ymat.*log(out)-(1-ymat).*log(1-out),2),1)/m+( sum(sum(Theta1(:,2:end).^2,2),1) ...
+ sum(sum(Theta2(:,2:end).^2,2),1))*lambda/(2*m);
